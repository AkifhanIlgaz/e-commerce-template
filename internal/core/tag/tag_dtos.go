...
// CreateTagRequest, Create servisinin girdisi. Handler formdan doldurur;
// servis içinde Tag modeline çevrilir.
...
// UpdateTagRequest, Update servisinin girdisi. ID handler tarafında
...
// ListTagsRequest, sayfalı listeleme girdisi. Page/PerPage sınır
...
func (r ListTagsRequest) Normalize() ListTagsRequest {
	if r.Page < 1 {
		r.Page = 1
	}
	if r.PerPage < 1 {
...
// TagPage, sayfalı listeleme çıktısı.
type TagPage struct {
	Items      []Tag
	Total      int64
	Page       int
