...
type Session struct {
	...
	CreatedAt time.Time `json:"created_at"` // absolute timeout için
	LastSeen  time.Time `json:"last_seen"`  // idle timeout için
}
...
func (m *SessionManager) Create(ctx context.Context, c fiber.Ctx, scope Scope, u *User) (*Session, error) {
	sess := &Session{
		ID:        randomToken(),
		UserID:    u.ID.Hex(),
		Email:     u.Email,
