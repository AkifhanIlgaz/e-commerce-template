...
func SessionFromCtx(c fiber.Ctx) *session.Session {
	s, _ := c.Locals(sessionLocalKey).(*session.Session)
	return s
}
...
func WithSession(sm *session.SessionManager, scope session.Scope) fiber.Handler {
...
// Admin paneli RequireRole(sm, AdminScope, RoleAdmin) ile korunur.
func RequireRole(sm *session.SessionManager, scope session.Scope, role string) fiber.Handler {
	return func(c fiber.Ctx) error {
